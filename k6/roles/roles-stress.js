...
export default function () {
    ...
    check(resRoles, {
        'GET roles status 200': (r) => r.status === 200,
    });

    // Creación ocasional (5% de las iteraciones)
    if (Math.random() < 0.05) {
        const nombreRol = `Stress_Rol_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`;
        const payloadCrear = JSON.stringify({
            nombre: nombreRol,
            descripcion: 'Rol creado en test de estrés'
        });
        const resCrear = http.post(`${BASE_URL}/api/roles`, payloadCrear, params);
        check(resCrear, {
            'POST crear rol stress status 201': (r) => r.status === 201,
        });
    }
